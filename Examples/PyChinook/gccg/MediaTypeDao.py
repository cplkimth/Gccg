

from typing import Sequence
from sqlalchemy import select, func, text, Select, TextClause
from gccg.EntityDao import EntityDao
from entities.MediaType_ import MediaType_
from models import MediaType

class MediaTypeDao(EntityDao[MediaType_]):
    # region override
    def _select(self) -> Select[tuple[MediaType_]]:
        return select(MediaType_)

    def _count(self) -> Select[int]:
        return select(func.count("*")).select_from(MediaType_)

    def _by_key(self, *keys) -> TextClause:
        return text(f"MediaTypeId = {keys[0]} ")

    def _order_by(self) -> TextClause:
        return text("MediaTypeId ")

    def _order_by_desc(self) -> TextClause:
        return text("MediaTypeId desc ")

    def copy(self, source: MediaType_, target: MediaType_) -> None:
        
        # target.MediaTypeId = source.MediaTypeId 
        
        target.Name = source.Name 

    def clone(self, source: MediaType_) -> MediaType_:
        target = self.clone()
        self.copy(source, target)

        return target

    # endregion

    def create(self) -> MediaType_:
        entity = MediaType_()
        self._create_core(entity)
        return entity

    # region by foreign key
    
    # endregion

